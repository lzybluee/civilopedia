...
if (window.location.pathname.indexOf('/en-us/') >= 0) v_chinese = false;
...
window.onload = function() {
	...
	var containers = getByClass("menugroupcontainer");

	for (var i = 0; i < containers.length; i++) {

		var div = containers[i].children[1];

		var found = false;
		for (var j = 0; j < div.children.length; j++) {
			if (window.location.href.indexOf(div.children[j].href) >= 0) {
				div.children[j].style.color = "aqua";
				found = true;
				break;
			}
		}
...
function makeHTML(text, blnimage, title) {
	...
	var path_modifier = v_chinese ? "" : "../";

	var container_style = ""; //'border:' + v_border + 'px solid ' + v_bordercolor + ';';
	//container_style += 'background-color:' + v_backcolor + ';';
	if (!blnimage) {
		container_style += "background-image: url(\'" + path_modifier + "images/tipcentre.png\');";
		container_style += "line-height: 27px;";
	} else {
		container_style += "background-image: url(\'" + path_modifier + "images/small/PROMOTION_BLANK.png\'); height: 64px; width: 64px;";
	}
	...
	if (!blnimage)
		text = "<img src=\'" + path_modifier + "images/tipleft.png\' style=\'height: 27px; width: 6px;\' />" + text + "<img src=\'" + path_modifier + "images/tipright.png\' style=\'height: 27px; width: 6px;\' />";

	if (text.indexOf("src=\'/civilopedia") >= 0) {
		text = text.replace("src=\'/civilopedia/", "src=\'" + path_modifier);
	}
...
function mouseX(evt) {
	if (evt.pageX) return evt.pageX;
		...
		return evt.clientX + ((document.documentElement && document.documentElement.scrollLeft) ?
			document.documentElement.scrollLeft :
			((document.body && document.body.scrollLeft) ? document.body.scrollLeft : 0));
	} else {
		return null;
	}
}
...
function mouseY(evt) {
	if (evt.pageY) {
		...
		return evt.clientY + ((document.documentElement && document.documentElement.scrollTop) ?
...
function createBackLayer(Z) {
	...
	return '<iframe id="backdrop" frameborder="0" scrolling="no" height="0" style="z-index: ' + Z + '; filter: alpha(Style=0,Opacity=0);"><p></iframe>';
}
...
function cboLanguage_SelectedIndexChanged() {
	...
	if (strLanguage != "") {
		...
		var strPage = strPath.substring(strPath.lastIndexOf('/') + 1);
		strPath = strPath.substring(0, strPath.lastIndexOf('/') + 1);
		var strURL = strPath + strPage.substring(3);
		v_chinese = true;
	}
...
function togglediv(strDiv) {
	...
	if(div.style.display == "") {
		...
		for (var i = 0; i < containers.length; i++) {
			if (div.parentNode != containers[i]) {
